...
import { ArrowDown, ChevronDown, Command, Download, GitBranch,Users } from "lucide-react";
// import { ArrowUpRight, Play, Search, Sparkles, } from "lucide-react";
...
//
function Logo() {
  return <div className="logo-mark" aria-label="Kizen home"><span className="logo-z">Z</span><span>zed</span><i /></div>;
}
//
  ...
  return (
    <main>
      <header className="site-header">
        ...
        <a className="announcement" href="#delta"><span>Stay tuned: multiplayer environment for workings with agents [in developement]</span> {/*<ArrowUpRight size={13} />*/}</a>
      </header>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-content">
          ...
          <h1>Your last pitstop for<br></br> "MOST" of your needs [:"]</h1>
          <p className="hero-sub">Kizen is a minimal task master crafted for<br className="desktop" /> solo devs.</p>
          <div className="hero-actions"><a href="#download" className="button-primary"><Download size={14} /> Download now <kbd>D</kbd></a><a href="https://github.com/samSRaina/kizen" className="button-secondary"><GitBranch size={14} /> Clone source <kbd>C</kbd></a></div>
      ...
      <section className="feature-strip">
        {
          [
            { title: "FREE", text: "Completely free to use, MIT liicensed and open source." },
            { title: "MODERATELY FAST", text: "Written from scratch in Go to efficiently leverage multi-core CPU's." },
            { title: "POSTGRES", text: "To maintain smooth state transitions at native speed." },
            { title: "REDIS [IN DEV]", text: "For caching." },
            { title: "SQLC", text: "single source of truth for database schemas" },
            { title: "OPEN API", text: "single source of truth for API definitions" },
            { title: "DOCKER", text: "One command to Launch" },
          ].map((item, i) => <div className="feature" key={item.title}><span className="feature-index">0{i + 1}</span><h3>{item.title}</h3><p>{item.text}</p></div>)}
      </section>
      ...
      <footer>
        <div className="footer-brand">
          <Logo /><p>Minimal task manager crafted for <a href="https://github.com/samsraina">@samRaina</a></p>
        </div>
        ...
        <div className="footer-bottom"><span>2026 Kizen</span><span>GitHub ↗</span></div></footer>
    </main>
  );
}
